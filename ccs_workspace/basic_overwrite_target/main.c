...
char* morse_code_from_char(char letter) {
...
int main(void)
{
    ...
    const int str_len = 27;

    char str[str_len] = "SPHINXOFBLACKQUARTZJUDGEMYVOW";


    while(1) {
        unsigned int j;
        for (j = 0; j < str_len; j++) {
            char* x = morse_code_from_char(str[j]);
            unsigned int i;
            for (i = 0; x[i] != 0; i++) {
                if (x[i] == '.') {
                    P1OUT |= BIT0;      // LED1 on
