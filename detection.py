...
class Detection:
    ...
    def __init__(self, path: str, file: str, *, pique_bin_bool: bool = True) -> None:
        self.hash: Dict[int: str] = None
        self.file: str = file
        self.path: str = path
        self.binary_security_quality: float = 0
        if pique_bin_bool:
            self.pique_bin: PiqueBin = PiqueBin(binary_file_name=self.file,
                                                binary_file_directory=self.path)
...
if __name__ == "__main__":
    detection: Detection = Detection(path=r"C:\Users\wward\Documents\GitHub\Raytheon_VHDL_Generator\ccs_workspace\test_generated_ASM", file="test_asm_generation.asm")
    detection.detect()
