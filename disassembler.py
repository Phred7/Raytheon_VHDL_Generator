...
class Disassembler:
    ...
    def pique_bin(self, binary_file_directory: str, binary_file_name: str) -> float:
        ...
        """
        pique_bin_package_directory: str = f"{os.getcwd()}\\PIQUE-Bin-Jar\\"  # added single slash before P
        pique_exit_status: int = self.check_malware(binary_file_directory=binary_file_directory,
                                                    binary_file_name=binary_file_name,
                                                    pique_bin_directory=pique_bin_package_directory)
        self.logger.debug(f"PIQUE-Bin exist status: {pique_exit_status}")
        if pique_exit_status != 0:
            raise OSError(
                f"PIQUE-Bin failed to run on {binary_file_name} with exit status {pique_exit_status}")
        else:
            self.logger.info(f'PIQUE-Bin successfully ran on {binary_file_name}')
        return self.pique_bin_score(binary_file_name, pique_bin_package_directory)

    def check_malware(self, binary_file_directory: str, binary_file_name: str, pique_bin_directory: str) -> int:
        """
        ...
        self.file_should_exist(binary_file_directory, binary_file_name)
    ...
    def pique_bin_score(self, binary_file_name: str, pique_bin_directory: str) -> float:
        """
        ...
        self.file_should_exist(pique_bin_output_file_directory, pique_bin_output_file)
    ...
    def disassemble(self, *, pique_bool: bool = True) -> Tuple[str, str]:
        """
        ...
        disassembler_input_file_name: str = "Motor_mover_C.out" # "All_msp_operations.out"  # "test_C.out"  # "test_colt_C.out"  # "All_ops_asm.out'  # "Motor_mover_C.out" # "test.out" # "test_ASM.out" # "All_msp_operations.out"
        ...
        self.file_should_exist(disassembler_input_file_directory, disassembler_input_file_name)

        # Run PIQUE-bin.
        if pique_bool:
            self.logger.info(
                f"PIQUE-Bin Binary Security Quality: {self.pique_bin(disassembler_input_file_directory, disassembler_input_file_name)}")
        ...
        if os.path.exists(rf"{disassembler_output_file_directory}\{disassembler_output_file_name}"):
            ...
            self.logger.info(rf"Removed {disassembler_output_file_directory}\{disassembler_output_file_name}")
        ...
        self.logger.debug(f"Disassembler exit status: {disassembler_exit_status}")
        if disassembler_exit_status != 0:
            ...
            self.logger.info(
                f'Disassembler generated {disassembler_output_file_name} at the directory {disassembler_output_file_directory}')
        return disassembler_output_file_directory, disassembler_output_file_name
